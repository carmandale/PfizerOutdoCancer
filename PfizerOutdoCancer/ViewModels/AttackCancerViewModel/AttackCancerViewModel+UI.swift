...
class AttackCancerUIViewModel {
    ...
    func setupUIAttachments(in root: Entity, attachments: RealityViewAttachments, count: Int) {
        ...
        Logger.debug("Total attachments to create: \(count)")
        
        for i in 0..<count {
            Logger.debug("Setting up attachment \(i)")
            if let meter = attachments.entity(for: "\(i)") {
                Logger.debug("✅ Found meter entity for \(i)")
                if root.findEntity(named: "cancer_cell_\(i)") != nil {
                    Logger.debug("✅ Found cancer cell \(i)")
                    root.addChild(meter)
                    meter.components[UIAttachmentComponent.self] = UIAttachmentComponent(attachmentID: i)
                    meter.components.set(BillboardComponent())
                    
                    Logger.debug("✅ Added meter to cancer_cell_\(i) with components")
                } else {
                    Logger.debug("❌ Could not find cancer cell \(i)")
                }
            } else {
                Logger.debug("❌ Could not create meter entity for \(i)")
            }
        }
    }
} 

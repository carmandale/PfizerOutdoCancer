...
struct ADCView: View {
    ...
    @State private var localVisible: Bool = true
    ...
    var body: some View {
        ...
        .opacity(opacity)
        .onAppear {
            if appModel.isBuilderInstructionsOpen && !didAnimate {
                withAnimation(.easeIn(duration: 1.5)) {
                    opacity = 1.0
                }
                didAnimate = true
            } else {
                // If instructions are no longer open, ensure the view stays hidden.
                opacity = 0.0
            }
        }
        .onDisappear {
